...
class DmCheckerTest {
    @Test
    fun `a room is a DM only if it has at most 2 members, is direct and encrypted`() {
        val isDirect = true
        val activeMembersCount = 2
        val isEncrypted = true

        val isDm = DmChecker.isDm(isDirect, activeMembersCount, isEncrypted)

        assertThat(isDm).isTrue()
    }

    @Test
    fun `a room is can be a DM if it has also a single active user`() {
        val isDirect = true
    ...
    @Test
    fun `a room is not a DM if it's not encrypted`() {
        val isDirect = true
        val activeMembersCount = 2
