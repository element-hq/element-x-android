 ...
 */
class MicrophoneAccessService : Service() {
    companion object {
        fun start(context: Context) {
            if (ContextCompat.checkSelfPermission(context, Manifest.permission.RECORD_AUDIO)
                == PackageManager.PERMISSION_GRANTED) {
                val intent = Intent(context, MicrophoneAccessService::class.java)
                ContextCompat.startForegroundService(context, intent)
            } else {
                Timber.w("Microphone permission is not granted, cannot start the microphone access service")
            }
        }
    ...
    override fun onCreate() {
        ...
        val notification = NotificationCompat.Builder(this, foregroundServiceChannel.id)
            ...
            // TODO i18n
            .setContentTitle("Microphone title")
            .setContentText("Microphone content")
            .setContentIntent(pendingIntent)
            .build()
        val notificationId = NotificationIdProvider.getForegroundServiceNotificationId(ForegroundServiceType.MICROPHONE)
        ...
        runCatching {
            ...
            // Note: this is happening on my device when using the task switcher :/
            Timber.e(it, "Failed to start microphone foreground service")
        }
    }
