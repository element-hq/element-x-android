...
interface MarkAllRoomsAsRead {
    suspend fun hasUnreadRooms(): Boolean
...
class DefaultMarkAllRoomsAsRead(
    ...
    override suspend fun hasUnreadRooms(): Boolean = withContext(coroutineDispatchers.io) {
        val unreadRoomList = createUnreadRoomList()
        unreadRoomList.updateFilter(RoomListFilter.Unread)
        unreadRoomList.awaitLoaded()
        val numberOfRooms = (unreadRoomList.loadingState.value as? RoomList.LoadingState.Loaded)?.numberOfRooms ?: 0
        numberOfRooms > 0
    }
