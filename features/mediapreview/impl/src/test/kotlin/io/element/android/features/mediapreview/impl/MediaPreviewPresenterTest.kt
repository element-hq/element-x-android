...
class MediaPreviewPresenterTest {
    ...
    @Test
    fun `present - image editor state is null by default`() = runTest {
        createMediaPreviewPresenter().test {
            val state = awaitItem()
            assertThat(state.imageEditorState).isNull()
            assertThat(state.isApplyingImageEdits).isFalse()
            assertThat(state.displayImageEditError).isFalse()
            cancelAndIgnoreRemainingEvents()
        }
    }
}
...
internal fun TestScope.createMediaPreviewPresenter(
    ...
    attachmentImageEditor: AttachmentImageEditor = mockk("attachmentImageEditor") {
        coEvery { canEdit(any()) } returns false
    },
