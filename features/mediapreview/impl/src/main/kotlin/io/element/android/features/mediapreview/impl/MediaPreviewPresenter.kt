...
import io.element.android.features.mediapreview.impl.imageeditor.AttachmentImageEditor
import io.element.android.features.mediapreview.impl.imageeditor.AttachmentImageEditorState
import io.element.android.features.mediapreview.impl.imageeditor.AttachmentImageEdits
...
class MediaPreviewPresenter(
    ...
    override fun present(): MediaPreviewState {
        ...
        var canEditImage by remember { mutableStateOf(localMedia.info.canEditImage()) }
        ...
        fun handleEvent(event: MediaPreviewEvents) {
            when (event) {
                ...
                MediaPreviewEvents.OpenImageEditor -> {
                    if (canEditImage) {
                        preprocessMediaJob?.cancel()
                ...
                MediaPreviewEvents.CloseImageEditor -> {
                    imageEditorState = null
                }
                is MediaPreviewEvents.UpdateImageCropRect -> {
                ...
                MediaPreviewEvents.ClearImageEditError -> {
                    displayImageEditError = false
                }
            }
        }
    ...
    private fun resetPreparedMedia(sendActionState: MutableState<SendActionState>) {
        val info: MediaUploadInfo? = sendActionState.value.mediaInfo()
        if (info != null) {
            info.allFiles().forEach { it.delete() }
        }
        mediaSender?.cleanUp()
        sendActionState.value = SendActionState.Idle
    }
