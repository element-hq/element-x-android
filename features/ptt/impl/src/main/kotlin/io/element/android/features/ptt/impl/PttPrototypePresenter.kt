 ...
 *
 * Drives the [PttSessionController] (the transport-agnostic session host) rather than a specific
 * backend: joining creates the room's transport via the factory and connects; the talk button
 * takes/releases the half-duplex floor. State is derived from the transport's live session state.
 */
@Inject
