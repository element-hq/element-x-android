...
class RoomMembersModerationPresenter @Inject constructor(
    ...
    override fun present(): RoomMembersModerationState {
        ...
        val unbanUserAsyncAction =
            remember { mutableStateOf(AsyncAction.Uninitialized as AsyncAction<RoomMember, Unit>) }
            ...
            when (event) {
                is RoomMembersModerationEvents.SelectRoomMember -> {
                    if (event.roomMember.membership == RoomMembershipState.BAN && canBan) {
                        unbanUserAsyncAction.value = AsyncAction.Confirming(event.roomMember)
                    } else {
    ...
    private fun CoroutineScope.unbanUser(
        userId: UserId,
        unbanUserAction: MutableState<AsyncAction<RoomMember, Unit>>,
    ...
    private fun <C, T> CoroutineScope.runActionAndWaitForMembershipChange(
        action: MutableState<AsyncAction<C, T>>,
