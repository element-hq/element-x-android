    ...
    override fun present(): TimelineProtectionState {
        ...
        fun handleEvent(event: TimelineProtectionEvent) {
            when (event) {
                ...
                is TimelineProtectionEvent.ValidateContent -> room.roomCoroutineScope.launch {
                    contentScannerService.scan(event.eventId, event.mediaSource) {
                        event.validationState.value = it
                    }
                }
            }
        }
